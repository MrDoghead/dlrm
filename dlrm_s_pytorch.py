...
class DLRM_Net(nn.Module):
    ...
    def sequential_forward(self, dense_x, lS_o, lS_i):
        # process self.bot_ldense features (using bottom mlp), resulting in a row vector
        # dense_x: bs x 13  lS_o: 26 x bs   lS_i: bs,
        x = self.apply_mlp(dense_x, self.bot_l)
        # x: bs x 64
        # debug prints
        # print("intermediate")
        ...
        #print("emb_list:",self.emb_l)
        #print("self.v_W_lL",self.v_W_l) #None
        ly = self.apply_emb(lS_o, lS_i, self.emb_l, self.v_W_l)
        # for y in ly:
...
def run():
    ...
    if args.save_onnx:
        ...
        # debug prints
        print("input shape:", X_onnx.shape)
        # export model
        #torch.save(X_onnx, "dense_x.pt")
        #torch.save(lS_o_onnx, "ls_o.pt")
        #torch.save(lS_i_onnx, "ls_i.pt")
        #print('3 files saved')
        #sys.exit()
        ...
        print('onnx saved at',dlrm_pytorch_onnx_file)
